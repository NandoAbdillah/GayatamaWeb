    ...
    <DashboardLayout title="Manajemen Pos Kebutuhan KKN Desa">
      <div className="space-y-6 font-jakarta">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-extrabold text-navy-950 font-epilogue">Pos Kebutuhan KKN Desa Sukamaju</h1>
        ...
        {filteredPos.length === 0 ? (
          <Card className="p-10 text-center bg-white border-slate-200">
            <p className="text-sm text-slate-500">Tidak ada pos kebutuhan yang sesuai pencarian / filter.</p>
          </Card>
        ) : (
            ...
            {filteredPos.map((pos) => (
              <Card key={pos.id} className="p-6 border-slate-200 bg-white flex flex-col justify-between space-y-4 shadow-ambient">
                <div className="space-y-3">
                  ...
                  <h3 className="text-base font-bold text-navy-950 font-epilogue leading-snug">{pos.judul}</h3>

                  <p className="text-xs text-slate-600 font-jakarta line-clamp-3 leading-relaxed">{pos.deskripsi}</p>
