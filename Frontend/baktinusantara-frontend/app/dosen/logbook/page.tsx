        ...
        <div className="flex flex-col lg:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-2xl text-sm text-navy-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary shadow-sm font-jakarta"
            />
          </div>
        ...
        <div className="space-y-4">
          {filteredLogs.length === 0 ? (
            <Card className="p-10 text-center bg-white border-slate-200">
              <p className="text-sm text-slate-500 font-jakarta">Tidak ada logbook yang sesuai dengan pencarian / filter.</p>
            </Card>
          ) : (
              ...
              return (
                <Card key={log.id} className="p-6 border-slate-200 bg-white space-y-4 shadow-ambient">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 border-b border-slate-100 pb-3">
                    <div className="space-y-1">
                      <div className="flex items-center gap-1.5 text-xs text-slate-500 font-medium">
                        ...
                        <strong className="text-navy-950">{getKelompokLabel(log)}</strong>
                      </div>
                      ...
                      <div className="flex gap-3 overflow-x-auto pb-2">
                        {(log.foto_dokumentasi_urls ?? []).map((url, i) => (
                          <img
                            ...
                            src={url}
                            alt="Dokumentasi"
                            className="w-24 h-24 object-cover rounded-xl border border-slate-200 shadow-sm shrink-0"
                          />
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="rounded-2xl bg-orange-50 border border-orange-200 overflow-hidden">
                      ...
                      {isExpanded && (
                        <div className="px-4 pb-4 -mt-1">
                          <p className="text-xs text-orange-950 leading-relaxed whitespace-pre-line">
                            {log.catatan_revisi_dpl}
                          </p>
                        </div>
                      )}
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-3 border-t border-slate-100">
                    <div className="flex items-center gap-1.5 text-xs text-slate-500 font-medium order-1">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-navy-950/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 sm:p-8 shadow-2xl border border-slate-200 space-y-5">
            ...
            <div className="text-center space-y-2">
              <h3 className="text-lg font-extrabold text-navy-950 font-epilogue">Sahkan Logbook Ini?</h3>
